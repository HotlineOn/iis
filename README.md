...
Сервис позволяет с помощью http запросов получить доступ к предсказаниям модели.
Сервис работает на связке FastAPI и uvicorn.
Также можно запустить в docker контейнере.

Для загрузки модели используется скрипт `services/models/get_model.py`.
Примечание: скрипт запускать из директории `services/models`.
Чтобы загрузить нужную модель необходимо ввести соответствующее значение
Run ID в переменную `RUN_NAME` в скрипте `get_model.py`.
Run ID модели нужно брать из MLFlow:
![run_id](services/models/run_id.png)

В директории `services/ml_service` находится код для "общения" с моделью средствами FastAPI.
В ней же находится `Dockerfile` для сборки образа сервиса.
Чтобы собрать образ с названием *estate_model* нужно ввести команду `docker build . --tag estate_model:0`.
Чтобы запустить контейнер с портом 8001 
вводим команду `docker run -p 8001:8000 -v $(pwd)/../models:/models estate_model:0`.

После запуска сервера можно проверить его работоспособность.
Так как необходимо проверять post запросы, то удобнее всего делать это по адресу http://localhost:8001/docs. 
Для проверки post запроса `/api/prediction` нужно нажать кнопку `try it now` и ввести входные параметры.
В поле `employee_id` можно ввести любое число целого типа.
В качестве тестового тела запроса для поля `item_features` можно использовать следующий json:
```
{
    "work_year": 2024,
    "experience_level": "SE",
...
# Развёртка группы сервисов

Для развёртки нескольких сервисов, предназначенных для последующего анализа реализованной модели,
используется docker compose, рецепт для которого находится в файле `services/compose.yml`. 
Также создан сервис для осуществления запросов к сервису модели, он находится в директории `services/requests`.
Ещё один сервис - prometheus, служит для сбора статистики в виде гистограммы предсказаний модели и вывода графиков,
отображающих загруженность вычислительной машины и некоторые данные по работе модели.
Для развёртки с помощью compose необходимо предварительно собрать все образы (команда прописаны с учётом того, что
текущая директория является директория проекта):
```
docker build services/ml_service --tag estate_model:2
docker build services/reuests --tag request_service:1
```

Также необходимо дополнительно установить следующие образы:
```
docker pull prom/prometheus
docker pull grafana/grafana
```

Чтобы запустить все контейнеры образов разом, необходимо прописать:
```
cd services
docker compose up
```